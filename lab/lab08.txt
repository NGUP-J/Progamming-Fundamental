    ...
    while (play)
    {
        ...
        if (numEvents != 0)
        {
            ...
            for (DWORD i = 0; i < numEventsRead; ++i)
            {
                ...
                {
                    int Xs = eventBuffer[i].Event.MouseEvent.dwMousePosition.X;
                    int Ys = eventBuffer[i].Event.MouseEvent.dwMousePosition.Y;
...
void clear_buffer()
{
    for (int y = 0; y < screen_y; ++y)
    {
        for (int x = 0; x < screen_x; ++x)
        {
            ...
            consoleBuffer[x + screen_x * y].Attributes = 7; // White color
        }
    }
}
